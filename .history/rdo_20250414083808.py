...
def fake_rerun(*args, **kwargs):
       st.sidebar.write("Rerun tentou ser chamado aqui!")
       import traceback
       st.sidebar.code(traceback.format_stack())
...
if "debug_execucoes" not in st.session_state:
    st.session_state["debug_execucoes"] = []

timestamp = time.time()
st.session_state["debug_execucoes"].append(timestamp)

# Mostra apenas as últimas 5 execuções
if len(st.session_state["debug_execucoes"]) > 1:
    ultimas = st.session_state["debug_execucoes"][-5:]
    intervalos = [round((ultimas[i] - ultimas[i-1])*1000) for i in range(1, len(ultimas))]
    
    st.sidebar.write("## Debug")
    st.sidebar.write(f"Execuções totais: {len(st.session_state['debug_execucoes'])}")
    st.sidebar.write(f"Intervalos (ms): {intervalos}")
...
def criar_rdo():
    ...
    with col2:
        adicionar_funcionario = st.button("Adicionar Funcionário")
        if adicionar_funcionario and novo_funcionario:
            st.session_state["equipe"].append(novo_funcionario)
            # Em vez de st.rerun() - usando st.experimental_rerun()
    ...
    for i, func in enumerate(st.session_state["equipe"]):
        ...
        with col1:
            st.write(f"{i+1}. {func}")
        with col2:
            remover_func = st.button(f"Remover", key=f"rem_func_{i}")
            if remover_func:
    ...
    for i, atividade in enumerate(st.session_state["atividades"]):
        ...
        if atividade['anexos']:
            ...
            for j, anexo_path in enumerate(atividade['anexos']):
                ...
                with col2:
                    remover_anexo = st.button(f"Remover anexo", key=f"rem_anexo_{i}_{j}")
                    if remover_anexo:
    ...
    adicionar_atividade = st.button("Adicionar outra atividade")
        ...
        if ocorrencia.get('anexos'):
            ...
            for j, anexo_path in enumerate(ocorrencia['anexos']):
                ...
                with col1:
                    st.write(os.path.basename(anexo_path))
                with col2:
                    remover_anexo_ocor = st.button(f"Remover anexo", key=f"rem_anexo_ocor_{i}_{j}")
                    if remover_anexo_ocor:
                        if os.path.exists(anexo_path):
                            try:
                                os.remove(anexo_path)
                            except:
    ...
    registrar_ocorrencia = st.button("Registrar Ocorrência")
    if registrar_ocorrencia:
...
def visualizar_relatorios():
    ...
    for id_rel, relatorio in relatorios_filtrados:
        ...
        if "visualizar_detalhes" in st.session_state:
