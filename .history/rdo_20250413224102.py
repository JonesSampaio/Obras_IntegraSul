...
def visualizar_relatorios():
    ...
    for id_rel, relatorio in relatorios_ordenados:
        # Converter string para datetime para comparação, tentando diferentes formatos
        try:
            ...
            try:
                ...
                data_rel = datetime.strptime(relatorio['data_relatorio'], "%d/%m/%Y").date()
            except ValueError:
                # Se ambos falharem, pula este relatório
                st.warning(f"Formato de data inválido no relatório {id_rel}")
                continue
    ...
    for id_rel, relatorio in relatorios_filtrados:
        with st.container():
            ...
            with col1:
                ...
                # Converter a data para formato brasileiro DD/MM/YYYY para exibição
                try:
                    data_formatada = datetime.strptime(
                        relatorio['data_relatorio'], "%Y-%m-%d").strftime("%d/%m/%Y")
                except ValueError:
                    try:
                        # Se já estiver no formato DD/MM/YYYY, mantém como está
                        data_temp = datetime.strptime(
                            relatorio['data_relatorio'], "%d/%m/%Y")
                        data_formatada = relatorio['data_relatorio']
                    except ValueError:
                        data_formatada = relatorio['data_relatorio']  # Mantém original se falhar
