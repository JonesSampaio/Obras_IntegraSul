...
def gerenciar_usuarios():
    ...
    with col2:
        ...
        if usuarios_filtrados:
            ...
            if usuario_selecionado:
                ...
                with col1:
                    if st.button("Redefinir Senha"):
                        sucesso, nova_senha = auth.redefinir_senha(usuario_selecionado)
                        if sucesso:
                            st.success(f"Senha de '{usuario_selecionado}' redefinida para: {nova_senha}")
                        else:
                ...
                with col2:
                    ...
                    if st.session_state.nivel_acesso == "admin":
                        if st.button("Excluir Usuário"):
                            if usuario_selecionado == st.session_state.usuario_atual:
            ...
            st.info("Nenhum usuário disponível para visualização.")
...
def gerenciar_sistema():
    ...
    if st.button("Fazer Backup de Todos os Dados"):
        try:
            data_atual = datetime.now().strftime("%Y%m%d_%H%M%S")
            backup_dir = f"backup_{data_atual}"
            
            if not os.path.exists(backup_dir):
                os.makedirs(backup_dir)
            
            # Copiar todos os arquivos de dados
            for arquivo in ["usuarios.json", "obras.json", "funcionarios.json", "equipes.json"]:
                if os.path.exists(f"dados/{arquivo}"):
                    with open(f"dados/{arquivo}", 'r') as f_src:
