...
def criar_rdo():
    """Interface para criação de novo RDO"""
    usuario_logado = verificar_usuario_logado()
    if not usuario_logado:
        st.warning("Você precisa estar logado para criar um RDO.")
        return
    
    st.title("Criar Novo RDO")
    
    # Carregar obras
    obras = carregar_obras()
    if not obras:
        st.error("Não foi possível carregar as obras. Verifique o arquivo obras.json.")
        return
    ...
    if obra_selecionada:
        ...
        with col1:
            st.write(f"**Data de Início:** {obra_info.get('data_inicio', 'N/A')}")
            st.write(f"**Prazo:** {obra_info.get('prazo', 'N/A')}")
            st.write(f"**Etapa Atual:** {obra_info.get('etapa_atual', 'N/A')}")
        with col2:
            st.write(f"**Endereço:** {obra_info.get('endereco', 'N/A')}")
            st.write(f"**RRT:** {obra_info.get('rrt', 'N/A')}")
            st.write(f"**Status:** {obra_info.get('status', 'N/A')}")
        ...
        with col1:
            clima_manha = st.selectbox(
                "Clima pela Manhã",
                options=["Ensolarado", "Nublado", "Chuvoso", "Nevando"],
                index=0
            )
        with col2:
        ...
        temperatura = st.number_input("Temperatura Média (°C)", min_value=-20, max_value=50, value=20)
        
        # Horários de trabalho
        st.subheader("Horários de Trabalho")
        col1, col2 = st.columns(2)
        with col1:
            horarios_manha = st.time_input("Início Manhã", value=datetime.strptime("08:00", "%H:%M").time())
            horarios_manha_fim = st.time_input("Fim Manhã", value=datetime.strptime("12:00", "%H:%M").time())
        with col2:
            horarios_tarde = st.time_input("Início Tarde", value=datetime.strptime("13:00", "%H:%M").time())
            horarios_tarde_fim = st.time_input("Fim Tarde", value=datetime.strptime("17:00", "%H:%M").time())
        
        # Equipamentos
        st.subheader("Equipamentos")
        equipamentos = st.text_area("Equipamentos Utilizados", height=100)
        obs_equipe = st.text_area("Observações da Equipe", height=100)
        ...
        nova_atividade = st.text_input("Nova Atividade")
        ...
        if st.button("Salvar RDO"):
            ...
            relatorio = {
                ...
                "temperatura": temperatura,
                "horarios": {
                    "manha_inicio": horarios_manha.strftime("%H:%M"),
                    "manha_fim": horarios_manha_fim.strftime("%H:%M"),
                    "tarde_inicio": horarios_tarde.strftime("%H:%M"),
                    "tarde_fim": horarios_tarde_fim.strftime("%H:%M")
                },
                "equipamentos": equipamentos,
                "obs_equipe": obs_equipe,
            ...
            if salvar_rdo(
                ...
                clima_tarde=clima_tarde,
                temperatura=temperatura,
                horarios_chuva_manha={},
                horarios_chuva_tarde={},
                equipe=[],
                equipamentos=equipamentos,
                obs_equipe=obs_equipe,
                atividades=st.session_state.atividades,
                ocorrencias=[],
                recebimento_materiais_sim=False,
                materiais_recebidos="",
                necessidade_materiais_sim=False,
                materiais_necessarios="",
                materiais_urgentes=False,
                observacoes_gerais="",
                usuario=usuario_logado
            ):
                st.success("RDO salvo com sucesso!")
                st.session_state.atividades = []  # Limpar atividades
                st.experimental_rerun()
            else:
                st.error("Erro ao salvar o RDO.")
...
def criar_rdo():
    ...
    with col1:
        funcionarios_disponiveis = [f"{func['nome']} - {func['cargo']}" for func in funcionarios.values() if func.get('ativo', True)]
