    ...
    with col1:
        ...
        if st.button("Entrar"):
            ...
            if usuario in usuarios and hash_senha(senha) == usuarios[usuario]["senha"]:
                ...
                if not st.session_state.get("login_redirecionado", False):
                    st.session_state.login_redirecionado = True
                    time.sleep(0.1)  # Pequeno delay para evitar loops
                    st.rerun()
            else:
...
def mudar_senha():
    ...
    if st.button("Salvar nova senha"):
        if nova_senha == confirmar_senha and len(nova_senha) >= 6:
            ...
            # Redireciona para a página principal
            if not st.session_state.get("senha_redirecionado", False):
                st.session_state.senha_redirecionado = True
                time.sleep(0.1)  # Pequeno delay para evitar loops
                st.rerun()
