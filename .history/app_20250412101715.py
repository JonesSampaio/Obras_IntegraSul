...
if not st.session_state.autenticado:
    ...
    st.caption(f"Usuário: {st.session_state.tipo_usuario}")
    ...
    with col1:
        ...
        st.caption(f"Data formatada: {data_formatada}")
    
    with col2:
        # Adicionar campo para hora
        hora_atual = datetime.now().strftime("%H:%M")
        hora = st.time_input("Hora", datetime.strptime(hora_atual, "%H:%M").time())
    ...
    with col_botoes1:
        if st.button("💾 Salvar Relatório", use_container_width=True):
            # Registrar data e hora atual no momento do salvamento
            ...
            relatorio = {
                "Data": data_formatada,
                "Hora": hora_formatada,
                "Data e Hora": data_hora_completa,
                "Obra": nome_obra,
