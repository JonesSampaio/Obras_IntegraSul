...
def gerenciar_usuarios():
    ...
    with st.expander("Adicionar Novo Usuário", expanded=False):
        with st.form("novo_usuario"):
            username = st.text_input("Nome de Usuário", help="Use apenas letras, números e underscore")
            nome_completo = st.text_input("Nome Completo")
            nivel = st.selectbox("Nível de Acesso", ["usuario", "gerente", "admin"], 
                               help="usuario: pode criar e visualizar RDOs\n"
                                    "gerente: pode gerenciar obras e funcionários\n"
            ...
            if nivel == "usuario":
                obras_atribuidas = st.multiselect("Obras Atribuídas", list(obras.keys()),
                                                help="Selecione as obras que este usuário poderá acessar")
            ...
            if submitted:
                if not username or not nome_completo:
                    ...
                    st.error("Nome de usuário deve conter apenas letras, números e underscore!")
    ...
    for username, dados in usuarios_filtrados.items():
        ...
        with st.expander(f"{username} ({dados['nivel']})"):
            ...
            st.write(f"**Data de Criação:** {dados.get('data_criacao', 'Não informada')}")
            
            if dados["nivel"] == "usuario" and "obras" in dados:
                st.write(f"**Obras Atribuídas:** {', '.join(dados['obras']) if dados['obras'] else 'Nenhuma'}")
