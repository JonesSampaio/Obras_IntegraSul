...
from auth_utils import verificar_usuario_logado
...
def gerar_pdf_rdo(id_relatorio):
    try:
        ...
        pdf.set_font("Arial", "", 12)
        pdf.cell(0, 10, f"Clima Manhã: {dados['clima_manha']}", ln=True)
        pdf.cell(0, 10, f"Clima Tarde: {dados['clima_tarde']}", ln=True)
        ...
        pdf.set_font("Arial", "", 12)
        if dados.get('equipe'):
            pdf.cell(0, 10, "Funcionários presentes:", ln=True)
            for func in dados['equipe']:
                pdf.cell(0, 10, f"- {func}", ln=True)
        if dados.get('obs_equipe'):
            pdf.cell(0, 10, f"Observações: {dados['obs_equipe']}", ln=True)
        
        # Equipamentos
        pdf.set_font("Arial", "B", 14)
        pdf.cell(0, 10, "4. Equipamentos", ln=True)
        pdf.set_font("Arial", "", 12)
        if dados.get('equipamentos'):
            pdf.multi_cell(0, 10, dados['equipamentos'])
        
        # Atividades
        pdf.set_font("Arial", "B", 14)
        pdf.cell(0, 10, "5. Atividades Realizadas", ln=True)
        pdf.set_font("Arial", "", 12)
        if dados.get('atividades'):
            for i, atividade in enumerate(dados['atividades'], 1):
                pdf.cell(0, 10, f"Atividade {i}:", ln=True)
                pdf.cell(0, 10, f"Descrição: {atividade.get('descricao', '')}", ln=True)
                pdf.cell(0, 10, f"Status: {atividade.get('status', '')}", ln=True)
                pdf.ln(5)
        
        # Ocorrências
        pdf.set_font("Arial", "B", 14)
        pdf.cell(0, 10, "6. Ocorrências", ln=True)
        pdf.set_font("Arial", "", 12)
        if dados.get('ocorrencias'):
            for i, ocorrencia in enumerate(dados['ocorrencias'], 1):
                pdf.cell(0, 10, f"Ocorrência {i}:", ln=True)
                pdf.cell(0, 10, f"Descrição: {ocorrencia.get('descricao', '')}", ln=True)
                pdf.cell(0, 10, f"Gravidade: {ocorrencia.get('gravidade', '')}", ln=True)
                pdf.cell(0, 10, f"Resolução: {ocorrencia.get('resolucao', '')}", ln=True)
                pdf.ln(5)
        
        # Materiais
        pdf.set_font("Arial", "B", 14)
        pdf.cell(0, 10, "7. Materiais", ln=True)
        pdf.set_font("Arial", "", 12)
        
        # Recebimento de materiais
        pdf.cell(0, 10, "Recebimento de materiais:", ln=True)
        if dados.get('recebimento_materiais_sim'):
            pdf.cell(0, 10, "Sim", ln=True)
            if dados.get('materiais_recebidos'):
                pdf.multi_cell(0, 10, f"Descrição: {dados['materiais_recebidos']}")
        else:
            pdf.cell(0, 10, "Não", ln=True)
        
        # Necessidade de materiais
        pdf.cell(0, 10, "Necessidade de materiais:", ln=True)
        if dados.get('necessidade_materiais_sim'):
            pdf.cell(0, 10, "Sim", ln=True)
            if dados.get('materiais_necessarios'):
                pdf.multi_cell(0, 10, f"Descrição: {dados['materiais_necessarios']}")
            if dados.get('materiais_urgentes'):
                pdf.cell(0, 10, "URGENTE: Necessário nas próximas 48 horas", ln=True)
        else:
            pdf.cell(0, 10, "Não", ln=True)
        
        # Observações Gerais
        pdf.set_font("Arial", "B", 14)
        pdf.cell(0, 10, "8. Observações Gerais", ln=True)
        pdf.set_font("Arial", "", 12)
        if dados.get('observacoes_gerais'):
            pdf.multi_cell(0, 10, dados['observacoes_gerais'])
...
def carregar_funcionarios():
    ...
    try:
        ...
        if not os.path.exists("data/funcionarios.json"):
            # Criar arquivo com alguns funcionários de exemplo
            funcionarios = {
                "1": {
                    "nome": "João Silva",
                    "cargo": "Pedreiro",
...
def carregar_obras():
    """Carrega as obras do arquivo JSON"""
    try:
        # Verificar se o diretório existe
        if not os.path.exists("data"):
            os.makedirs("data")
            st.warning("Diretório 'data' criado.")
        
        # Verificar se o arquivo existe
        if not os.path.exists("data/obras.json"):
            st.warning("Arquivo obras.json não encontrado. Criando arquivo com obras de exemplo...")
            obras = {
                "Esc Est Med Adelina Isabela Konzen": {
                    "data_inicio": "2024-01-15",
                    "prazo": "2024-12-31",
                    "etapa_atual": "Fundações",
                    "endereco": "ROD Rsc 287 Km 68, nº Sn, CEP 95815-000 - Venâncio Aires",
                    "rrt": "10747",
                    "responsavel_principal": "Eng. Carlos Silva",
                    "responsavel_acompanhamento": "Eng. Ana Oliveira",
                    "status": "Em andamento"
                }
            }
            with open("data/obras.json", "w", encoding='utf-8') as f:
                json.dump(obras, f, indent=4, ensure_ascii=False)
            return obras
        
        # Tentar ler o arquivo
        with open("data/obras.json", "r", encoding='utf-8') as f:
            obras = json.load(f)
            if not obras:
                st.warning("Arquivo obras.json está vazio.")
                return {}
            return obras
            
    except json.JSONDecodeError as e:
        st.error(f"Erro ao decodificar o arquivo obras.json: {str(e)}")
        return {}
    except Exception as e:
...
def criar_rdo():
    ...
    if not funcionarios:
        st.warning("Nenhum funcionário cadastrado. Cadastre funcionários antes de criar um RDO.")
        return
    ...
    with col1:
        # Lista de funcionários disponíveis formatada corretamente
        funcionarios_disponiveis = [f"{func['nome']} - {func['cargo']}" for func in funcionarios.values()]
        novo_funcionario = st.selectbox("Selecionar funcionário:", options=[""] + funcionarios_disponiveis)
    with col2:
        if st.button("Adicionar Funcionário", key="add_func") and novo_funcionario:
            if novo_funcionario not in st.session_state["equipe"]:
                st.session_state["equipe"].append(novo_funcionario)
                st.success(f"Funcionário {novo_funcionario} adicionado à equipe!")
                st.rerun()
            else:
                st.warning("Este funcionário já está na equipe!")
        ...
        for i, func in enumerate(st.session_state["equipe"]):
            ...
            with col2:
                if st.button(f"Remover", key=f"rem_func_{i}"):
                    ...
                    st.success("Funcionário removido da equipe!")
                    st.rerun()
...
def visualizar_relatorios():
    ...
    # Se estiver visualizando detalhes, não mostrar a lista
            ...
            with col4:
                ...
                if pdf_path and os.path.exists(pdf_path):
                    ...
                    st.download_button(
                        label="Download PDF",
                        data=pdf_bytes,
                        file_name=f"RDO_{relatorio['numero_rdo']}_{id_rel}.pdf",
                        mime="application/pdf",
                        key=f"pdf_{id_rel}"
                    )
...
def main():
    ...
    # Adicionar Painel Administrativo se for admin
    if usuario_logado["admin"]:
        opcao = st.sidebar.radio(
            "Navegação",
            ["Criar novo RDO", "Visualizar Relatórios", "Editar Relatório", "Dashboard", "Painel Administrativo"],
            index=0
        )
