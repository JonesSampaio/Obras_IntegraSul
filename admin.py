...
def gerenciar_usuarios():
    ...
    with st.form("novo_usuario_form"):
        ...
        nivel_acesso = st.selectbox(
            "Nível de Acesso",
            ["admin", "gerente", "arquiteto", "engenheiro", "usuario"]
        )
    ...
    if usuarios_filtrados:
        for usuario, dados in usuarios_filtrados.items():
            ...
            nome_completo = dados.get("nome_completo", "Não especificado")
            nivel_acesso = dados.get("nivel_acesso", "usuario")
            status = "Ativo" if dados.get("ativo", True) else "Inativo"
            
            with st.expander(f"{usuario} - {nome_completo} ({nivel_acesso})"):
                col1, col2 = st.columns(2)
                with col1:
                    st.write(f"**Nome Completo:** {nome_completo}")
                    st.write(f"**Nível de Acesso:** {nivel_acesso}")
                    st.write(f"**Status:** {status}")
                with col2:
                    ...
                    # Botão para desativar/ativar com confirmação
                    if st.button("Desativar/Ativar", key=f"toggle_{usuario}"):
                        dados['ativo'] = not dados.get('ativo', True)
                        salvar_usuarios(usuarios)
                        st.success(f"Status do usuário {usuario} alterado para {'Ativo' if dados['ativo'] else 'Inativo'}")
                    
                    # Botão para excluir com confirmação
                    if st.button("Excluir Usuário", key=f"delete_{usuario}"):
                        if st.warning("Tem certeza que deseja excluir este usuário? Esta ação não pode ser desfeita."):
                            if st.button("Confirmar Exclusão", key=f"confirm_delete_{usuario}"):
